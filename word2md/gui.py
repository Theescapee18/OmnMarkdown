...
class OmnMarkdownApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.root.minsize(1100, 800)  # 最小尺寸限制（防止缩小后按钮消失）
    ...
    def _build_convert_tab(self, parent):
        ...
        self.task_count_label = ttk.Label(file_frame, text="当前任务: 0 / 10", font=("Microsoft YaHei", 18))
        ...
        self.file_listbox = tk.Listbox(list_frame, selectmode=tk.EXTENDED, yscrollcommand=scrollbar.set, font=("Microsoft YaHei", 18), height=5)
        ...
        ttk.Label(dir_row, text="保存位置:", font=("Microsoft YaHei", 18)).pack(side=tk.LEFT)
        self.output_entry = ttk.Entry(dir_row, textvariable=self.output_dir, width=40, font=("Microsoft YaHei", 18))
        ...
        self.status_label = ttk.Label(action_frame, text="就绪", foreground="gray", font=("Microsoft YaHei", 18))
    ...
    def _build_scan_tab(self, parent):
        ...
        ttk.Label(add_row, text="添加敏感词:", font=("Microsoft YaHei", 18)).pack(side=tk.LEFT)
        ttk.Entry(add_row, textvariable=self.new_word_entry, width=30, font=("Microsoft YaHei", 18)).pack(side=tk.LEFT, padx=(6, 6))
        ttk.Label(add_row, text="分类:", font=("Microsoft YaHei", 18)).pack(side=tk.LEFT)
        ttk.Entry(add_row, textvariable=self.new_word_category, width=10, font=("Microsoft YaHei", 18)).pack(side=tk.LEFT, padx=(6, 6))
        ...
        ttk.Button(add_row, text="打开词库", command=self._open_wordlist_file, width=8).pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(add_row, text="清空词库", command=self._clear_wordlist, width=8).pack(side=tk.LEFT, padx=(6, 0))
        ...
        self.wordlist_box = tk.Listbox(list_frame, yscrollcommand=wl_scroll.set, font=("Microsoft YaHei", 18), height=6, selectmode=tk.EXTENDED)
        ...
        ttk.Label(target_row, text="扫描目标:", font=("Microsoft YaHei", 18)).pack(side=tk.LEFT)
        ttk.Entry(target_row, textvariable=self.scan_target, width=40, font=("Microsoft YaHei", 18)).pack(side=tk.LEFT, padx=(6, 6), fill=tk.X, expand=True)
        ...
        self.scan_result_text = tk.Text(result_frame, yscrollcommand=res_scroll.set, font=("Microsoft YaHei", 18), height=10, state=tk.DISABLED)
        ...
        ttk.Label(report_row, text="报告保存:", font=("Microsoft YaHei", 18)).pack(side=tk.LEFT)
        ttk.Entry(report_row, textvariable=self.scan_report_dir, width=40, font=("Microsoft YaHei", 16)).pack(side=tk.LEFT, padx=(6, 6), fill=tk.X, expand=True)
    ...
    def _open_wordlist_file(self):
        """用系统默认编辑器打开词库文件"""
        scanner = SensitiveWordScanner()
        wordlist_path = scanner.wordlist_path
        if not wordlist_path.exists():
            messagebox.showinfo("提示", f"词库文件不存在：{wordlist_path}\n添加敏感词后会自动创建。")
            return
        try:
            os.startfile(str(wordlist_path))
        except Exception as e:
            messagebox.showerror("打开失败", f"无法打开词库文件：{e}\n路径：{wordlist_path}")

    def _clear_wordlist(self):
        """清空整个词库"""
        scanner = SensitiveWordScanner()
        count = scanner.get_word_count()
        if count == 0:
            messagebox.showinfo("提示", "词库已经是空的。")
            return
        if not messagebox.askyesno("确认清空", f"确定要清空词库中的全部 {count} 个敏感词吗？\n此操作不可撤销。"):
            return
        # 删除所有词
        categories = scanner.list_words()
        all_words = []
        ...
        removed = scanner.remove_words(all_words)
        messagebox.showinfo("完成", f"已清空词库，删除 {removed} 个敏感词。")
        self._refresh_wordlist()
...
def main():
    ...
    # 全局字体配置（大字体，清晰可读）
    FONT_LABEL = ("Microsoft YaHei", 18)
    FONT_BUTTON = ("Microsoft YaHei", 18)
    FONT_TITLE = ("Microsoft YaHei", 28, "bold")
    FONT_LIST = ("Microsoft YaHei", 18)
    FONT_STATUS = ("Microsoft YaHei", 18)
    FONT_START_BTN = ("Microsoft YaHei", 22, "bold")

    style.configure("TLabel", font=FONT_LABEL)
    style.configure("TButton", font=FONT_BUTTON, padding=12)
    style.configure("TCheckbutton", font=FONT_LABEL)
    style.configure("TLabelframe.Label", font=("Microsoft YaHei", 20, "bold"))
    style.configure("TEntry", font=FONT_LABEL, padding=10)
    style.configure("TSpinbox", font=FONT_LABEL, padding=10)
    style.configure("Horizontal.TProgressbar", thickness=32)
    style.configure("Start.TButton", font=FONT_START_BTN, padding=20)
    style.configure("TNotebook.Tab", font=("Microsoft YaHei", 18, "bold"), padding=[20, 10])
